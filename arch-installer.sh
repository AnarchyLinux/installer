...
check_connection() {
	...
	(wget --no-check-certificate --append-output=/tmp/wget.log -O /dev/null "http://speedtest.wdc01.softlayer.com/downloads/test10.zip"
...
manual_partition() {
	...
	### Else if the manual_part variable contains a number 0-9 this means it is a partition
		...
		### prompt user to create a new mountpoint on selected partition
			...
			if [ "$mnt" == "SWAP" ]; then
				if (whiptail --title "$title" --yes-button "$yes" --no-button "$no" --yesno "$swap_frmt_msg" 11 50) then
					(wipefs -a -q /dev/"$part"
					mkswap /dev/"$part"
					...
					swapon /dev/"$part" &> /dev/null
					if [ "$?" -gt "0" ]; then
						whiptail --title "$title" --ok-button "$ok" --msgbox "$swap_err_msg2" 10 60
					fi
				fi
			...
			### Else if mount is not equal to swap
				...
				if [ "$(</tmp/ex_status.var)" -gt "0" ]; then
					whiptail --title "$title" --ok-button "$ok" --msgbox "$part_err_msg2" 10 60
				fi
			fi
		fi
	...
	### Else if manual part variable is set to 'done'
		...
		### Else partition is mounted, create a list and count of final partitions
			...
			final_part=$(lsblk | grep "/\|[SWAP]" | grep "part" | grep -v "/run" | awk '{print $1"      "$4"       "$7}' | sed 's/\/mnt/\//;s/\/\//\//;1i'$partition': '$size':       '$mountpoint': ' | sed "s/\,/\./;s/\.[0-9]*//;s/ [0-9][G,M]/&   /;s/ [0-9][0-9][G,M]/&  /;s/ [0-9][0-9][0-9][G,M]/& /")
	...
	if "$mounted" ; then	
		...
		if ! "$wifi" ; then
			if (whiptail --title "$title" --defaultno --yes-button "$yes" --no-button "$no" --yesno "$wifi_option_msg" 11 60) then
				base_install="$base_install wireless_tools wpa_supplicant wpa_actiond networkmanager"
				enable_nm=true
			fi
		else
			base_install="$base_install wireless_tools wpa_supplicant wpa_actiond networkmanager"
...
graphics() {
	...
	### If the user did not enter from the menu prompt user to install a desktop
	### If user selects no ask to confirm not installing a desktop and continue to next function
	if ! "$menu_enter" ; then
	...
	### User didn't select cancel "de_set" is true
	else
		de_set=true
	fi
	...
	### Set "DE" variable to selected DE and lightdm
		...
		### .xinitrc will be created later on for selected desktop
		else
			whiptail --title "$title" --ok-button "$ok" --msgbox "$startx_msg" 10 60
		fi
	fi

	### If no desktop has been installed set the "DE" variable to include xorg-server and selected GPU
	if ! "$desktop" ; then
		if "$enable_nm" ; then
	...
	### (NOTE: For some reason this only worked when I output into a file I couldn't get the pacstrap command to work for some reason when 
	### trying to set its output to a variable, because of this I've had to create a "go-around", output into file and set "download_size" varable to output of file)
	### "software_size" variable is used to display size of packages to be downloaded to user (adds "MiB")
	### The "cal_rate" function is used to calculate the total download time based on connection and processor speed the "download_size" variable is used in "cal_rate"
	...
	### Prompt user to confirm installing desktop display packages to be downloaded, size, and estimated time
	### If user confirms run pacstrap command to install desktop
	...
	if (whiptail --title "$title" --yes-button "$install" --no-button "$cancel" --yesno "$desktop_confirm_var" 18 60) then
		...
		### Add .xinitrc file to skel directory and to root's home directory
		echo "$start_term" > "$ARCH"/etc/skel/.xinitrc
		echo "$start_term" > "$ARCH"/root/.xinitrc
	
	### User selected no to installing new desktop - display warning message
	else
		if ! "$menu_enter" ; then
	...
	### Continue to install_software function
	install_software
