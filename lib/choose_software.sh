...
# TODO: check if items are in array (since we don't use strings anymore)
    ...
    while (true); do
        ...
        case "${software_menu}" in
            ...
            "${done_msg}")
                if [[ -z "${final_software}" ]]; then
                    ...
                    # download = list of all the packages to install
                    ...
                    if [[ ! "${menu_enter}" ]]; then
                        ...
                        if (dialog --yes-button "${install}" --no-button "${cancel}" --yesno "\n${software_confirm_var1}" "${height}" 65); then
                            # Installs the packages in an arch-chroot
        ...
        if "${add_soft}"; then
            if [[ -z "${software}" ]]; then
                ...
                # Add all packages from packages_file to software_list array
                readarray -t software_list < "${packages_file}"

                size=$(pacman -Sy --print-format='%s' "$(echo "${software_list[@]}")") | awk '{s+=$1} END {print s/1024/1024}'

                download_size=$(pacman -Sy --print-format='%s' "$(echo "${software_list[@]}")") | awk '{s+=$1} END {print s/1024/1024}'
                pid=$! pri=0.1 msg="${wait_load} \n\n \Z1> \Z2pacman -Sy --print-format\Zn" load

                software_size=$(<<<"${download_size}" sed 's/\(\..\)\(.*\)/\1 MiB/')
                software_int="${#software_list[@]}" # Number of packages to install
                source "${lang_file}"
